...
class Godoc {
  constructor (goconfigFunc, gogetFunc) {
    ...
    this.key = 'reference'
    this.isViewActive = null
    this.debouncer = null
    this.timeoutActive = false
    this.docFollowsCursor = true
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      this.subscriptions.add(editor.onDidChangeCursorPosition((evt) => {
        if (!evt.cursor) {
          return
        }
        if (!this.docFollowsCursor) {
          return
        }
        ...
        this.debouncer = setTimeout(() => { this.doneMoving(evt.cursor) }, 400)
  ...
  getDoc (file, offset, cwd, cmd, stdin) {
    ...
    this.updateContent('Generating documentation...')
